...
export default removeUndefinedLocaleResources({
  ...
  'banners.release.action.add-to-bundle': '번들에 추가',
  ...
  'banners.variant.not-in-variant':
    '<PerspectiveTitle>{{perspectiveTitle}}</PerspectiveTitle>에 대한 <VariantBadge>{{variantTitle}}</VariantBadge> 변형 문서가 존재하지 않습니다.',
  ...
  'document-target-badges.not-in-target.tooltip':
    '선택한 perspective에 아직 문서가 존재하지 않습니다.',
  ...
  'document-view.form-view.sync-stalled.description':
    '변경 사항이 저장될 때까지 편집이 일시 중지됩니다. 변경 사항은 브라우저에 보관되므로 재시도하는 동안 이 탭을 열어 두세요.',
  ...
  'incoming-references-pane.no-references': '수신 참조를 찾을 수 없습니다.',
  ...
  'panes.document-operation-results.operation-success_publishVersion': '버전이 게시되었습니다',
