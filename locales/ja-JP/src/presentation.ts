...
export default removeUndefinedLocaleResources({
  ...
  'preview-frame.configuration.error.description':
    'プレビュー iframe は <Code>{{targetOrigin}}</Code> を読み込むように設定されていますが、報告されたオリジンは <Code>{{reportedOrigin}}</Code> です。Presentation Tool はセキュリティ上の理由から不明なオリジンへの接続ができません。Visual Editing および Loader への接続を許可するには、<Code>presentationTool.allowOrigins</Code> の設定を更新してください。',
  ...
  'preview-frame.configuration.error.title': 'プレビュー URL オリジンの不一致',
  ...
  'preview-frame.loader.connection-status.label': 'Loader の接続ステータス',
  ...
  'preview-search-param.configuration.error.description':
    'ルーターは<Code>{{previewSearchParam}}</Code>へのナビゲーションを試みていますが、オリジン<Code>{{blockedOrigin}}</Code>は許可されていません。許可するには<Code>presentationTool.allowOrigins</Code>の設定を更新してください。',
  ...
  'preview-url-secret.missing-grants':
    'URLプレビューシークレットを作成する権限がありません。これによりプレビューの読み込みが失敗する可能性があります。',
