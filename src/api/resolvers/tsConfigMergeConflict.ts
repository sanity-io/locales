...
/**
 * Conflicts tend to arise when adding/removing locale packages to the `paths` key.
 ...
 *
 * @param content - The content of the `tsconfig.json` file, with potential merge conflicts
 ...
 * @internal
 */
export const tryHandleTsConfigMergeConflict: JsonParseErrorResolver = (content, err) => {
  ...
  for (const line of lines) {
    if (line.startsWith('<<<<<<<')) {
      ...
      inConflict = false
    } else if (inConflict && pathEntryRegex.test(line)) {
